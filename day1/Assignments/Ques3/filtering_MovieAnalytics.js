...
// 2. Filter those with rating >= 8
const topRated = sciFiMovies.filter(movie => movie.rating >= 8);

// 3. Sort by year (oldest → newest)
const sortedMovies = topRated.sort((a, b) => a.year - b.year);

// 4. Calculate average rating using reduce()
const avgRating = (sortedMovies.reduce((sum, movie) => sum + movie.rating, 0) / sortedMovies.length).toFixed(2);

// 5. Return movie titles and average
const resultTitles = sortedMovies.map(movie => movie.title);

console.log("Filtered Movies:", resultTitles);
